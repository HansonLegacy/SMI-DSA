...
sys.path.insert(0, '/home/jiaxuan/tmp_model/MoSt-DSA')
import config as most_cfg
from Trainer import Model as MostModel
...
@register('most_dsa')
class MostDSA(BaseModel):
    def __init__(self, cfgs):
        super(MostDSA, self).__init__(cfgs)
        
        model_args = cfgs.get('model', {}).get('args', {})
        
        most_cfg.MODEL_CONFIG['MODEL_ARCH'] = most_cfg.init_model_config(
            F=model_args.get('F', 32),
            lambda_range=model_args.get('lambda_range', 'local'),
            depth=model_args.get('depth', [2, 2, 2, 4])
        )
        
        self.most_model = MostModel(local_rank=-1)
    ...
    def forward(self, img0, img1, time_step=None, imgt=None, run_with_gt=False, **kwargs):
        ...
        img0 = img0[:, [2, 1, 0], :, :]  # RGB to BGR
        img1 = img1[:, [2, 1, 0], :, :]  # RGB to BGR
        
        if isinstance(time_step, torch.Tensor):
            time_step = time_step.squeeze()
        
        with torch.no_grad():
            if isinstance(time_step, (int, float)) or (isinstance(time_step, torch.Tensor) and time_step.dim() == 0):
                padder = InputPadder(img0.shape, divisor=32)
                img0_pad, img1_pad = padder.pad(img0, img1)
                pred = self.most_model.inference(img0_pad, img1_pad, timestep=time_step.item() if isinstance(time_step, torch.Tensor) else time_step, TTA=self.use_tta)
            ...
            pred = pred[:, [2, 1, 0], :, :]  # BGR to RGB
            
        return {'imgt_pred': pred}

    def train_one_step(self, input_dict):
        raise NotImplementedError("MostDSA does not support training")

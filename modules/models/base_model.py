...
class BaseModel:
    def __init__(self, cfgs):
        ...
        # ######## 补充: 冻结实验（还没有细配置） ########
        # def set_trainable(model, train_keywords):  
        #     for n, p in model.named_parameters():
        #         p.requires_grad = any(k in n for k in train_keywords)  # 这里是个判别，所有可train的部分设置为True，否则为False
        
        # 只有 bim_vfi 模型才需要 make_components 创建主模型
        model_name = self.cfgs['model'].get('name', '')
        if model_name == 'bim_vfi':
    ...
    def _log_learning_rates(self, step, tag_prefix="Train/LR"):
        lr_dict = self._get_lr_dict()
        if not lr_dict:
            return

        first_lr = next(iter(lr_dict.values()))
        if self.cfgs.get('enable_wandb', False):
            log_dict = {'lr': first_lr}
            for group_name, lr in lr_dict.items():
                log_dict[f'lr/{group_name}'] = lr
            wandb.log(log_dict, step=step)
        else:
            self.summary_writer.add_scalar(tag_prefix, first_lr, step)
            for group_name, lr in lr_dict.items():
                self.summary_writer.add_scalar(f'{tag_prefix}_{group_name}', lr, step)
    ...
    def train_one_step(self, input_dict):
        ...
        for k, v in self.loss_dict.items():
            ...
            if hasattr(v, "get_last_metrics"):
                extra_metrics = v.get_last_metrics()
                for metric_name, metric_value in extra_metrics.items():
                    losses[f"{k}_{metric_name}"] = metric_value
            loss += losses[k]
    ...
    def validate(self, val_loader, test_indicator=None):
        ...
        for input_dict in self.metric_logger.log_every(val_loader, print_freq, header):
            ...
            if (self.cfgs['mode'] == 'train' and self.current_epoch % self.cfgs['vis_every'] == 0) or (
                ...
                flow_t1_vis = flow2img(result_dict['flowfwd'])
                flow_t0_vis = flow2img(result_dict['flowbwd'])
                for i in range(len(scene_names[1])):
                    ...
                    save_image(imgt_pred[i], scenet_path.replace('.', '_pred.'))
                    save_image(imgt[i], scenet_path)
                    save_image(img1[i], scene1_path)
                    save_image(warped_img0[i], scenet_path.replace('.', '_warped_0.'))
                    save_image(warped_img1[i], scenet_path.replace('.', '_warped_1.'))
                    save_image(refine_mask[i], scenet_path.replace('.', '_refine.'))
                    save_image((img1[i] + img0[i]) / 2, scenet_path.replace('.', '_overlayed.'))
                    save_image(flow_t1_vis[i], scenet_path.replace('.', '_flow_t1.'))
                    save_image(flow_t0_vis[i], scenet_path.replace('.', '_flow_t0.'))
                    save_image(torch.abs(imgt[i] - imgt_pred[i]) * 10, scenet_path.replace('.', '_residual_x10.'))
        # gather the stats from all processes
        ...
        for i, dataset in enumerate(dataset_list):
            for split in split_list[i]:
                ...
                for input_dict in self.metric_logger.log_every(val_dataset, print_freq, header):
                    ...
                    # imgt_pred = (imgt_pred * 255.).round() / 255.  # 对齐most的结果，去掉round操作（26年2月26日补）
                    ...
                    self.cfgs['mode'] != 'train' and self.cfgs['benchmark_dataset']['save_imgs']):
                        ...
                        save_image(torch.abs(imgt[0] - imgt_pred[0]) * 10, scenet_path.replace('.', '_residual_x10.'))
                self.logger.info(f"Averaged validate stats:{self.metric_logger.print_avg()}")
                if (self.cfgs['mode'] == 'train' and self.current_epoch % self.cfgs['vis_every'] == 0) or (
                        self.cfgs['mode'] != 'train' and self.cfgs['benchmark_dataset']['save_imgs']):
                    psnr_str = []
    ...
    def move_components_to_device(self, mode):
        ...
        if self.stlpips_metric is not None:
    ...
    def count_parameters(self):
        ...
        if getattr(self.model_without_ddp, "use_adams_encoder", False):
            return model_number

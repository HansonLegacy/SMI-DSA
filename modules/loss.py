...
class PhotoTeacherLoss(nn.Module):
    ...
    def forward(self, interp_imgs_tea, imgt, **kwargs):
        ...
        return (loss_ternary.mean()*1.0 + loss_charbonnier.mean()*0.5) * self.weight
...
@register('weighted_bim')
class WeightedBiMLoss(nn.Module):
    def __init__(
        self,
        weight,
        gamma,
        ...
        vessel_sharpness=8.0,
        motion_threshold=0.1,
        motion_sharpness=12.0,
        r_weight=1.0,
        phi_weight=1.0,
    ):
        super().__init__()
        ...
        self.vessel_sharpness = vessel_sharpness
        self.motion_threshold = motion_threshold
        self.motion_sharpness = motion_sharpness
    ...
    def _build_vessel_map(self, img0, img1, target_hw):
        img0_gray = F.interpolate(
            self._gray(img0), size=target_hw, mode='bilinear',
            align_corners=False, antialias=True)
        img1_gray = F.interpolate(
            self._gray(img1), size=target_hw, mode='bilinear',
            align_corners=False, antialias=True)
        vessel_map = torch.maximum(img0_gray, img1_gray)
        vessel_map = self._normalize_per_sample(vessel_map)
        return torch.sigmoid((vessel_map - self.vessel_threshold) * self.vessel_sharpness)

    def _build_motion_map(self, flow0, flow1):
        motion_mag = torch.norm(flow0.detach(), dim=1, keepdim=True) + torch.norm(flow1.detach(), dim=1, keepdim=True)
        return torch.sigmoid((motion_mag - self.motion_threshold) * self.motion_sharpness)

    def _weighted_mean(self, loss_map, weight_map):
    ...
    def forward(
        ...
        **kwargs
        ...
        num_levels = min(
            ...
            len(bim_phi_gt_list),
            len(bim_r_pred_list),
            len(bim_phi_pred_list),
            len(flowt0_res_tea_list),
            len(flowt1_res_tea_list),
        )
        if num_levels == 0:
            self.last_metrics = {
                'r': img0.new_tensor(0.0),
                'phi': img0.new_tensor(0.0),
                'vessel_mean': img0.new_tensor(0.0),
                'motion_mean': img0.new_tensor(0.0),
            }
            return img0.new_tensor(0.0)

        loss = img0.new_tensor(0.0)
        r_loss_total = img0.new_tensor(0.0)
        phi_loss_total = img0.new_tensor(0.0)
        vessel_mean_total = img0.new_tensor(0.0)
        motion_mean_total = img0.new_tensor(0.0)
        for lvl in range(num_levels):
            r_gt = bim_r_gt_list[lvl].detach()
            r_pred = bim_r_pred_list[lvl]
            phi_gt = F.normalize(bim_phi_gt_list[lvl].detach(), dim=1, eps=1e-6)
            phi_pred = F.normalize(bim_phi_pred_list[lvl], dim=1, eps=1e-6)

            target_hw = r_pred.shape[-2:]
            vessel_map = self._build_vessel_map(img0, img1, target_hw)
            motion_map = self._build_motion_map(flowt0_res_tea_list[lvl], flowt1_res_tea_list[lvl])
            weight_map = 1.0 + self.vessel_weight * vessel_map + self.motion_weight * motion_map
            weight_map = weight_map + self.joint_weight * vessel_map * motion_map

            r_loss = ((r_pred - r_gt) ** 2 + 1e-6) ** 0.5
            phi_loss = 1.0 - (phi_pred * phi_gt).sum(dim=1, keepdim=True).clamp(-1.0, 1.0)

            level_r_loss = self.r_weight * self._weighted_mean(r_loss, weight_map)
            level_phi_loss = self.phi_weight * self._weighted_mean(phi_loss, weight_map * motion_map)
            level_loss = level_r_loss + level_phi_loss
            loss = loss + level_loss * (self.gamma ** lvl)
            r_loss_total = r_loss_total + level_r_loss * (self.gamma ** lvl)
            phi_loss_total = phi_loss_total + level_phi_loss * (self.gamma ** lvl)
            vessel_mean_total = vessel_mean_total + vessel_map.mean()
            motion_mean_total = motion_mean_total + motion_map.mean()

        self.last_metrics = {
            'r': (r_loss_total * self.weight).detach(),
            'phi': (phi_loss_total * self.weight).detach(),
            'vessel_mean': (vessel_mean_total / num_levels).detach(),
            'motion_mean': (motion_mean_total / num_levels).detach(),
        }
        return loss * self.weight
